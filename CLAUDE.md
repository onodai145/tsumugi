...
- `store/` — persistence. Settings and drafts are plain JSON files. The note cache defaults to SQLite (`rusqlite`) but is pluggable behind the `NoteCacheBackend` trait (Issue #115 Phase 2/3) — PostgreSQL and MySQL backends are also available, switchable from the settings UI (`commands/cache_backend.rs`, `domain/cache_backend.rs`); Postgres/MySQL credentials go through the keyring like account tokens, not through settings JSON.
- `session/` — account/token management; tokens go through the OS secret store, never through the frontend. Uses `keyring-core` + a platform-specific store crate directly (`apple-native-keyring-store` / `windows-native-keyring-store` / `zbus-secret-service-keyring-store` / `android-native-keyring-store`) — deliberately not the `keyring` crate's v1-compatible wrapper, since that has no Android support (see `session/secrets.rs`).
- `commands/` — the `#[tauri::command]` handlers, grouped by resource (`account`, `app`, `cache_backend`, `clip`, `column`, `draft`, `haptics`, `mute`, `note`, `sound`, `user`).
