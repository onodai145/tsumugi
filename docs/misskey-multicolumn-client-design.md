...
| Misskey APIバインディング | **REST型は手書きの薄い型付きラッパ + 接続ロジックは自前実装** | MisskeyはOpenAPI仕様（`/api-doc.json`）を公開しているが3.1.0であり、`progenitor`等の生成ツールは3.0.x専用でparseに失敗するため不採用（§6.1）。リクエスト/レスポンスの型定義は手書きラッパ（`src-tauri/src/api/`）で実装する。WebSocketの接続確立・チャンネル多重化・再接続・認証といった**接続ロジックは自前実装**とし、外部crate（`misskey-rs`等）にもmisskey-js（JS/TS）にも依存しない。これによりトークンをRustプロセス内に閉じ込めたまま実装できる（misskey-js案で懸念したWebViewへのトークン露出を回避） |
...
MisskeyはOpenAPI仕様（`/api-doc.json`）を公開している。当初は以下のビルドフローを想定していた。

1. 対象Misskeyバージョンの`api-doc.json`をリポジトリ内にスナップショットとして保存（サーバー起動時に毎回取得はしない。バージョン差異を意図せず取り込まないため）
2. `progenitor`（Oxide Computer製）を採用し、`build.rs`経由でコード生成する。Rust純正で継続的にメンテナンスされており、型だけでなく非同期reqwestベースのクライアントも同時に生成される点が決め手だった。

> **【確定】progenitorは不採用。** Phase 1で実機検証した結果、Misskey（misskey.io 2025.4.1）の`/api.json`は**OpenAPI 3.1.0**であり、progenitor 0.8が依存する`openapiv3` crate（OpenAPI 3.0.x専用）では3.1.0の`type: ["string","null"]`（配列型nullable）をparseできず失敗する（`invalid type: sequence, expected a string`）。progenitorはDropshot（Oxide自身のフレームワーク）が出力するOpenAPIドキュメントを主要ターゲットとしており、Misskey側の生成系との相性問題だった。
>
> 代わりにREST クライアントは**手書きの薄い型付きラッパ**（`src-tauri/src/api/`）として実装した。Misskey REST は全 POST・JSONボディに`i`(token) 同梱で均一なため手書きコストは低い。`api-doc.json`スナップショットは`src-tauri/openapi/misskey-api-doc.json`に保持している（将来3.1対応ジェネレータや3.1→3.0ダウンコンバートを導入する場合の入力用）。

**ストリーミングAPI（WebSocketメッセージのやり取り）はOpenAPI仕様の対象外**のため、こちらは公式ドキュメント・実装記をもとに型を手書きする必要がある。
...
1. ~~**`progenitor`がMisskeyの`api-doc.json`を問題なく処理できるかの実機検証が未実施。**~~ **【解決済み】** Phase 1で検証済み。OpenAPI 3.1.0非対応のため不採用とし、手書き型付きラッパ（`src-tauri/src/api/`）で実装した（§6.1参照）。
