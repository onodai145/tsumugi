...
- Android実機で以下を確認する:
  ...
  - デスクトップ版(`cargo tauri dev`)で、タブ切替・カラム分割・幅リサイズなど既存の挙動に
    変化が無いこと。
  - あるタブでノートをタップして選択状態にしてから、別のタブへスワイプする → 選択したノートの
    あるタブへ引き戻されたり、意図せず切り替わったりしないこと(Finding 1の確認)。
  - ノート一覧を一番下までスクロールしたとき、引き続き無限スクロール(追加読み込み)が発火する
    こと(縦スクロールの実要素が変わったため)。
  - タブが1つしかないカラムで横にスワイプすると、そのカラム内での引っかかりなく直接カラム移動に
    なること。
  - ネストした分割(下方分割/右方分割)があるレイアウトで、着地したカラムに正しくフォーカス
    リング/ハイライトが付くこと(Finding 2の確認)。
