...
2. Confirm all jobs (`check`, `cleanup`, `build` ×3, `android`) succeed.
3. Open the resulting `nightly` Release on GitHub and confirm: tagged `nightly`, marked **Pre-release** (not Latest), assets are named `tsumugi-nightly-...` (no `-main-` anywhere), and the Android APKs are present and signed (installable, not "package appears to be corrupt").
4. Run **Run workflow** a second time with no intervening commits to `main`; confirm the `check` job reports `should_run=false` and every other job is skipped (visible as "skipped" in the Actions run summary).
