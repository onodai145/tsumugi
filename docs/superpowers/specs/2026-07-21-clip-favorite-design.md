...
- お気に入り一覧を表示する専用カラム
- 起動時のお気に入り状態バックフィル(`notes/state` の一括版が Misskey に無いため)。副作用として、カラム再読込後は表示上「未お気に入り」に戻るため、再度トグルすると Misskey 側は ALREADY_FAVORITED/NOT_FAVORITED を返すことがある。この場合はエラー表示せず楽観的な表示状態をそのまま確定させる(`toggleFavorite` 参照)。
