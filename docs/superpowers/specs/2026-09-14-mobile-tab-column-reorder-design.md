...
- `pointermove`で座標更新。タイマー成立前に閾値（16px）を超えたら`cancelled`を返す
...
1. **長押し検知**: `pointerdown`（`pointerType === "touch"`）で`longPressDrag.ts`のタイマーを開始。
   16px以上動くか、タイマー成立前に`pointerup`が来たら中断し、これまで通りタップ（`onclick`での
...
1. **ドラッグ開始**: 長押し成立で`vibrate("light")`。画面上部に「◀ 前へ / 次へ ▶」のヒント
   オーバーレイを表示する。
2. **ドラッグ中**: 指のX移動量（開始位置からの差分）が閾値（80px）を超えたら該当方向
...
- 実際のタッチ長押し・`pointercancel`・ネイティブスクロールとの共存については、jsdomでは
  ...
  当初の実装では`touch-action: none`を指定しているのはカラムのグリップだけで、タブ要素側には
  指定していなかった。しかしAndroid実機での確認で、タブの長押し待機中（400ms成立前）の
  わずかな指のブレでもブラウザが早期に横スクロールへ確定してしまい、我々の`preventDefault()`
  （`armed`成立後にしか呼んでいない）が間に合わず、ネイティブスクロールとJS側のドラッグ処理
  （`translateX`・`dragOverTab`）が同時に走って動作が不安定になることが実際に確認された。この
  ため、**タブ要素にも`touch-action: none`を指定する**方針に変更した。トレードオフとして、
  タブの上を指でスワイプしてタブバーを横スクロールする操作はできなくなる（スクロールは
  タブバーの空き領域からのみ可能。オートスクロール未対応とあわせて、多数タブがある場合の
  操作性はv1の既知の制限とする）。

  - タブ: 長押しがタブバーの横スクロールに競り負けないこと（実機で確認済み）。
  - カラム: グリップの長押しドラッグが、カラム間のScroll Snap（横スワイプ）に競り負けないか。

  さらに、タブの長押し中に指がタブバーの外へ僅かにずれただけで、その位置にある
  **ノート本文**（`NoteCard.svelte`でコピー用途のため意図的に`select-text`にしている）が
  OS標準の「長押しでテキスト選択」により選択状態になる、という実機報告があった。`tab要素`
  自体は`select-none`済みだが、この抑制は対象要素の内部にしか効かないため、隣接する
  選択可能要素での選択発火は防げない。対処として、長押し成立（`armed`）からドラッグ終了
  までの間、`document.documentElement`に`user-select: none`/`-webkit-user-select: none`
