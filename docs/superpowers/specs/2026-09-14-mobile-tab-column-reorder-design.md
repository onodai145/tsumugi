...
- 実際のタッチ長押し・`pointercancel`・ネイティブスクロールとの共存については、jsdomでは
  ...
  ただし今回の長押し方式は、指が静止している間（移動が始まる前）にタイマーで先取り判定する
  ため、Issue #296で問題になった「早期の軸判定によるネイティブスクロールへの横取り」
  （ブラウザが移動の軌跡から判定して`pointercancel`を発行する現象）とは性質が異なる。
  長押し成立前に指が動けば単に中断してネイティブスクロール/タップに委ねるだけであり、
  スクロールとの競合による機能不全のリスクは低いと考えられるが、実機確認では特に
  「タブバーの横スクロール中に長押し判定が誤発火しないか」を重点的に確認する。
