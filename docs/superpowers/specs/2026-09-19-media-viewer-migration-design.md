...
- 画面左右端の縦中央にある透明クリックエリア（幅64px×高さ112px、ホバー時に矢印アイコンを表示、クリックで同上）。領域を全高・15%幅にすると、画像の左右余白のクリックで閉じられなくなる（3.8節）ため、矢印ボタン周りだけに絞っている
- キーボード ← / →（同上）
- 横スワイプ（3.9節のCSS Scroll Snapによるネイティブスクロール。JSのpointerドラッグ横取りは使わない）

### 3.8 共通操作

- Esc キー / 背景クリックで閉じる。ページ内のメディア本体（画像の表示範囲・動画/音声プレイヤー）とボタン以外の余白クリックでも閉じる。画像のパンなどで押下位置から6px超動いた後のclickは閉じない。
- タッチの縦スワイプで閉じる（等倍時のみ）。100px超で閉じ、足りなければ戻る。ズーム中（画像は`translatable`属性、動画は`data-zoomed`属性で判定）、2本指操作中（ピンチ）、コントロールバー/ボタン上から始めた操作、マウス操作は対象外。水平方向は前後送り（CSS Scroll Snap）に任せ、縦方向に確定した時だけ追従する。
