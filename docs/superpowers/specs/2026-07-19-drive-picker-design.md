...
- ファイル種別によるフィルタはかけない。Misskey の投稿添付ファイル自体に種別制限は無いため、ドライブの
  全ファイルを一覧表示する（画像以外は既存 `ComposeBar` のサムネイル表示同様、MIMEタイプ先頭部分の
  バッジ表示にフォールバック）。
...
- `list_files` は `POST /api/drive/files` に `{ i, folderId, untilId, limit: 30 }`（種別フィルタなし。
